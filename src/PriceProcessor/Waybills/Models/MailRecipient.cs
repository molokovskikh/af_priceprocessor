	...
	public class MailRecipient : ActiveRecordLinqBase<MailRecipient>
	{
		...
		public List<User> GetUsers(ulong regionMask)
		{
			...
			if (Type == RecipientType.Region && ((Region.Id & regionMask) > 0))
				result = User.Queryable.Where(u => (u.WorkRegionMask & Region.Id) > 0).ToList();

			if (Type == RecipientType.Client)
				result = User.Queryable.Where(u => u.Client.Id == Client.Id && (u.WorkRegionMask & regionMask) > 0).ToList();

			if (Type == RecipientType.Address)
				result = User.Queryable.Where(u => u.AvaliableAddresses.Any(a => a.Id == Address.Id) && (u.WorkRegionMask & regionMask) > 0).ToList();
