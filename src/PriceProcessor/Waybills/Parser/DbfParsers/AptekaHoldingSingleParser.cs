	...
	public class AptekaHoldingSingleParser : IDocumentParser
	{
		...
		public Document Parse(string file, Document document)
		{
			...
			document.Lines = data.Rows.Cast<DataRow>().Select(r => {
				...
				line.SupplierCostWithoutNDS = Convert.IsDBNull(r["TZENA"]) ? null : (decimal?)Convert.ToDecimal(r["TZENA"], CultureInfo.InvariantCulture);
                ...
                if (data.Columns.Contains("AFORDER"))
                    line.OrderId = Convert.IsDBNull(r["AFORDER"]) ? (uint?)null : Convert.ToUInt32(r["AFORDER"]);
