...
namespace PriceProcessor.Test.Waybills
{
	public class WaybillServiceIntegrationFixture : IntegrationFixture
	{
		...
		public void DocumentSetIdWithParentSynonymTest()
		{
			...
			product = new TestProduct("Виагра (табл.)");
			product.CatalogProduct.Pharmacie = true;
			session.Save(product);

			producer = new TestProducer("Пфайзер");
			session.Save(producer);
			session.Save(new TestSynonym() { Synonym = "Виагра", ProductId = product.Id, PriceCode = (int?)price.Id });

			session.Save(new TestSynonymFirm() { Synonym = "Пфайзер", CodeFirmCr = (int?)producer.Id, PriceCode = (int?)price.Id });
