...
namespace PriceProcessor.Test.Waybills.Parser.Txt
{
	...
	public class SiaVoronezhParserFixture
	{
		...
		public void Parse()
		{
			...
			Assert.That(doc.Lines[0].SupplierPriceMarkup, Is.EqualTo(-2.78));
			Assert.That(doc.Lines[0].RegistryCost, Is.EqualTo(311.90));
			Assert.That(doc.Lines[0].ProducerCostWithoutNDS, Is.EqualTo(297.34));
			...
			Assert.That(invoice.RecipientAddress, Is.EqualTo("ОГУП \"Липецкфармация\", 398059, г.Липецк, ул.Неделина, д.31\"а\", нежилое помещение №1"));
