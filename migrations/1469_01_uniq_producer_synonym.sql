...
BEGIN
	...
	if LastProducerSynonymId is null then
		...
		if IsAutomatic and ProducerId is null then
			insert into AutomaticProducerSynonyms(ProducerSynonymId, PriceCode, SynonymText) values(LastProducerSynonymId, PriceCode, Synonym);
		end if;
	end if;
